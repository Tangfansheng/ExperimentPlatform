...
namespace ExperimentTableDetectSystem.service
{
    public class PcanHelper
    {
        ...
        {
            double[] v = new double[77];//对外的实际值
            TPCANMsg CANMsg;
            TPCANTimestamp CANTimeStamp;
            TPCANStatus stsResult;

            for (int i = 0; i < 77; i++) { v[i] = 0; }
            do
            {
        ...
        public double d1(byte a)
        {
            return Math.Round(Convert.ToDouble(a),2);
        }
        public double d2(byte a,byte b)
        {
            return Math.Round(Convert.ToDouble(a) * 100 + Convert.ToDouble(b),2);
