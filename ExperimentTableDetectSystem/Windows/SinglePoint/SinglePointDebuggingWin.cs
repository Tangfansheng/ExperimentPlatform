        ...
        private void Fresh()
        {
          ...
          //  if (metroToggle22.Checked) { canmsg203.DATA[5] = 1; }
           // else { canmsg203.DATA[5] = 0; }
            ...
            //回油卸荷阀
            TPCANMsg canmsg206;
            canmsg206 = new TPCANMsg();
            canmsg206.ID = 0x206;
            canmsg206.LEN = Convert.ToByte(8);
            canmsg206.MSGTYPE = TPCANMessageType.PCAN_MESSAGE_STANDARD;
            canmsg206.DATA = new byte[8];
            if (metroToggle22.Checked) { canmsg206.DATA[0] = 1; }
            else { canmsg206.DATA[0] = 0; }
            peakhelper.write(canmsg206);
