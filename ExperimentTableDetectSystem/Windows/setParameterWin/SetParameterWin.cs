    ...
    public partial class SetParameterWin : MetroFramework.Forms.MetroForm
    {
        ...
        private void WriteToPlc()
        {
          ...
          //  canmsg102.DATA[3] = Convert.ToByte(a[3]);         //泄露升降阀口保压时间
            ...
            Double.TryParse(txtMainP.Text.Trim(), out d[0]);
            Double.TryParse(txtSteeringP.Text.Trim(), out d[1]);
            Double.TryParse(txtChangeLeakageP.Text.Trim(), out d[2]);
            Double.TryParse(txtOverloadP.Text.Trim(), out d[3]);
            for (int i = 0; i < 4; i++)
            {
                d[i] = d[i] * 10;
                canmsg111.DATA[2 * i] = Convert.ToByte((int)d[i] / 100);
                canmsg111.DATA[2 * i + 1] = Convert.ToByte((int)d[i] % 100);
            }    
            ...
            canmsg107.DATA = new byte[8];
                ...
                Double.TryParse(txtPump1Pressure.Text.Trim(), out g[0]);
                Double.TryParse(txtPump2Pressure.Text.Trim(), out g[1]);
                 
                for (int i = 0; i < 2; i++)
                {
                g[i] = g[i] * 10;
                    canmsg107.DATA[2 * i] = Convert.ToByte((int)g[i] / 100);
                    canmsg107.DATA[2 * i + 1] = Convert.ToByte((int)g[i] % 100);
                }         
            #endregion
